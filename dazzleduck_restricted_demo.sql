...
SET VARIABLE tenant_token = (
    SELECT dd_login(
        ...
        'admin',
        'admin',
        '{"database":"demo_catalog","schema":"main","table":"tenant","filter":"tenant_id = ' || CAST(getvariable('tenant_id') AS VARCHAR) || '"}'
    )
);
...
SET VARIABLE user_token = (
    SELECT dd_login(
        ...
        'admin',
        'admin',
        '{"database":"demo_catalog","schema":"main","table":"user","filter":"tenant_id = ' || CAST(getvariable('tenant_id') AS VARCHAR) || '"}'
    )
);
...
SET VARIABLE transaction_token = (
    SELECT dd_login(
        ...
        'admin',
        'admin',
        '{"database":"demo_catalog","schema":"main","table":"transaction","filter":"tenant_id = ' || CAST(getvariable('tenant_id') AS VARCHAR) || '"}'
    )
);
...
CREATE OR REPLACE VIEW tenant AS
    SELECT * FROM dd_read_arrow(
        'http://localhost:8081',
        source_table := 'demo_catalog.main.tenant',
...
CREATE OR REPLACE VIEW "user" AS
    SELECT * FROM dd_read_arrow(
        'http://localhost:8081',
        source_table := 'demo_catalog.main.user',
...
CREATE OR REPLACE VIEW "transaction" AS
    SELECT * FROM dd_read_arrow(
        'http://localhost:8081',
        source_table := 'demo_catalog.main.transaction',
